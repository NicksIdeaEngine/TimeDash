...
function RunPulseTime()
  ...
  if RescueTimeJSON['color'] ~= nil then
...
function RunCodeWarsRank(style)
  CodeWarsJSON = JSON:decode(SKIN:GetMeasure('GetCodeWarsRank'):GetStringValue())
  
  data = CodeWarsJSON['ranks']['overall']['name']
  
  SKIN:Bang('!SetOption', 'CodeWars', 'Text', data)
end
